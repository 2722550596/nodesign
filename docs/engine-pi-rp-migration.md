...
- **agent-dir 内容**：`settings.json`（**不设 defaultTools**——白名单是 pi 工具面的冻结快照，pi 以后加内建工具全拿不到；删掉后走 pi 默认激活集 read/bash/edit/write + state_update/get_state/subagent/subagent_profiles，自动跟随 pi 更新。要禁单个工具用 CLI `--exclude-tools, -xt`（denylist，args.ts:136），不造白名单。grep/find/ls 主会话不激活（bash 覆盖），子代理自带（prepare.ts:168）；telemetry off；`defaultPreset: "nodesign"` 双保险）、`prompt-presets/nodesign.json`（M2 第一步填全量平台提示词）。**禁止 SYSTEM.md / APPEND_SYSTEM.md**（短路 preset 编译）。
...
**开放项**（非阻塞，M3/簇 B 复评）：
- ~~bash 工具：pi defaultTools 白名单暂不含~~ → 已解决（2026-08-27）：先补 bash 进白名单，随后
