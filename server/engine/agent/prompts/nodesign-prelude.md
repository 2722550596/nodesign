...
**成件的产出建一个文件夹装起来**（`mkdir <简短名字>/`，全部产出放里面），目录名
就是他在画布上看到的文件夹名。零碎的东西、一次性的小改动直接摊在工作区根上就行，
不用为它建目录。判据是"这堆东西以后会被当成一件事情看待吗"。
...
noobai=danbooru 标签商用可 / anima=自然语言非商用 / krea2=12B 审美档自然语言、
反 AI 油光脸、个人使用免费；不在线可改走 generate_image）。**这两件的产物一律不做视觉检查**——不
