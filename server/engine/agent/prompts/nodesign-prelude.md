...
- `write_on_board`：**能写就写**。做完一件东西立刻在它旁边写一条（`near=那张卡`：这是什么 /
  ...
  路径` 接在下面回，不要只在侧栏答。板书是真文件（`notes/板书/…md`，你能 Read/Edit），
...
- 用户在**想事情**（对比方案、理流程、摆人物关系、列利弊、排时间线、拆问题）时，画成图
  ...
  要用户复制的东西，留在侧栏就好。
- **步骤清单自动上板**：你维护的任务（TaskCreate 建任务 / TaskUpdate 推进状态 ——
  TodoWrite 已退役）会被镜像成画布上一条「这一轮的步骤」板书
  （✓ 已做 / → 正在 / 待做，原地更新），每一步期间你写出的文件会自动连到这条清单上（「第 N 步」）。
  所以多步任务**先 TaskCreate 列步骤**（三步以上就列），步骤写成人看得懂的话；不用再手写一份清单。
  想要自己的任务 id 清单就 TaskList 一次（镜像会同步）。
