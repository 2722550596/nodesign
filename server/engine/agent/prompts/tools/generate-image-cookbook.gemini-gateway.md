# generate_image cookbook（Gemini gateway 版 · 已停用，不注入）

> ⛔ **这份文件不再被注入**。2026-08-17 起 cookbook 换成了写给 codex + gpt-image-2 的
> 版本（同目录 `generate-image-cookbook.md`），因为默认后端 2026-07-27 就已经换成 codex
> 订阅生图，而本文大量篇幅在教 Gemini/Nano-Banana-2 专属旋钮：`imageSize` /
> `thinkingLevel` / `model` 的 flash-pro 路由 / Image Search Grounding / PDF 文档输入 /
> NB2 model card 失败模式。这些参数在 codex 后端上**传了不报错也没有任何效果**，
> 留着只会让 agent 花心思去拨一个不存在的开关（还不会收到报错）。
>
> 本文原样保留，供 `NODESIGN_IMAGE_PROVIDER=gateway` 重新启用时取回。注入是按文件名
> 精确加载的（`loadToolPrompt('generate-image-cookbook')`），所以这个别名文件不会被读到。
>
> 以下为停用前原文。
