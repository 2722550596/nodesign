 ...
 *
 * ⭐ **起手文件**：加载 rp-craft skill 时平台自动把它拷进工作区根，
 * agent 只需 `cp 演出.template.js <演出文件夹>/演出.js`。**一个字都别改** ——
 * 这里面全是协议和踩过的坑，不是设计。
 * （为什么不放 patterns/ 让 agent 自己读：真跑实测它会 `find /` 全盘搜，
 * 搜出生产仓和开发 worktree 两份再自己猜哪份对 —— 今天字节相同侥幸没事，
 * 哪天两边不同步就会静默抄错版本。起手文件由平台投递，没有猜的余地。）
