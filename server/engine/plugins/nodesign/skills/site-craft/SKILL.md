...
1. 先用搜索找到目标站的 URL（浏览器不管搜索）
2. `browser_navigate` 打开 → `browser_read` 看结构和站内链接 → `browser_click`
   翻到内页。**能翻子页**这件事很关键：一个站的气质往往在内页而不是首页
3. ⭐ `browser_capture` 把可复用的带回来 —— 不只是截图：
   **调色板**（带用途：底色/正文色/强调色）、**字体**（字族 + 实际字号字重行高）、
   某一块的 **CSS 片段**、以及 **结构三数**
4. 结构三数正好是上面「用户说无聊的时候」那节要数的三样。⭐ **拿参照站的数字跟你
   正在做的那个站比**：「这个站 6 种节的形状，你现在的站 1 种」比「我觉得有点单调」
   能落地得多。（那三个数是启发式，当对照不当真相。）
5. 采下来的东西落在 `assets/references/web/`，**下个会话还在**，出处记在同目录的
   `.meta/<同名>.json` 里。**开工前先看看有没有现成的**，别重复逛一遍。
