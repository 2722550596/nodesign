...
`multiple`**：multiple 乘的是字体自带行高，预览替身（≈1.24 倍字号）和真 Word
的字体（微软雅黑≈1.32、Mac 苹方≈1.4）不一样高——预览里刚好排满的页，真 Word
里会多出几行、尾巴溢成新页（2026-08-19 实证：2 页简历在 Word 里变 4 页）。
换算参考：想要预览现在的行高，`exact ≈ 字号 × 原倍数 × 1.24`。
