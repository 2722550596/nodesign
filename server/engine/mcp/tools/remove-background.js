...
// 三档全开 alpha matting（消 halo / 色边伪影核心手段，只多 ~1-2× 时间但质量
// 提升明显）。fast/balanced/best 主要差在 model 大小 + 推理时间。
//
// 通过 service 路径 warm 时间（onnxruntime session 已在内存）：
//   fast:     isnet-general-use (170MB) + AM     — ~5-10s
//   balanced: birefnet-general-lite (214MB) + AM — ~10-20s
//   best:     birefnet-general (880MB) + AM      — ~20-40s（首次下 880MB ~3-5min）
const QUALITY_MAP = {
  fast:     { model: 'isnet-general-use',     alphaMatting: true },
...
Implementation: server-side spawn into a Python venv running rembg (BiRefNet
ML segmentation by default; trimap alpha matting post-process for clean edges).
Default quality "balanced" uses birefnet-general-lite (~3-5s warm) with alpha
matting on — produces SOTA-tier edges with minimal halo / 伪影.

QUALITY OPTIONS (tradeoff: speed vs edge fidelity):
- fast:     isnet-general-use, no alpha matting (~1-2s warm). Good when you'll
            apply the cutout at small size or the subject has a high-contrast bg.
- balanced: birefnet-general-lite + alpha matting (~3-5s warm). DEFAULT —
            produces clean edges suitable for hero placement on any canvas color.
- best:     birefnet-general (full SOTA) + alpha matting (~5-10s warm). For
...
Returns: text caption with output path + image content block (preview the result).`,
    {
      ...
      quality: z
        ...
        .optional()
        .describe('Speed/edge-fidelity tradeoff. Default: "balanced" (birefnet-general-lite + alpha matting, ~3-5s warm, clean edges). "fast" (isnet-general-use, no alpha matting, ~1-2s warm) for batch / non-critical. "best" (birefnet-general 880MB + alpha matting, ~5-10s warm; first call downloads model ~3-5min) for critical hero shots.'),
    },
    async ({ inputPath, outputName, overwrite = false, quality = 'balanced' }) => {
      ...
      // 每次 spawn 都是 cold：python startup + onnx import + model load + inference。
      // 实测 cold 时间（非 warm）：fast ~30-60s / balanced ~60-120s / best ~120-300s（首次还要下 880MB）。
      // timeout 按 quality 阶梯，给充裕余量。warm-cache 长期方案是 long-running python subprocess，待后续。
      const qualityCfg = QUALITY_MAP[quality];
      const timeoutByQuality = {
        fast: 90_000,       // 1.5min
        balanced: 180_000,  // 3min（birefnet-lite + AM cold）
        best: 600_000,      // 10min（首次含 880MB 下载）
      };
      const t0 = Date.now();
